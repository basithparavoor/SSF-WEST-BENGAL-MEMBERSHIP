...
// ==========================================
// INTERCEPT EXISTING HTML DOWNLOAD BUTTON
// ==========================================
window.downloadDigitalCardAsPDF = function(event) {
    if(event) event.preventDefault();
    if(activeCardId) promptDownloadDigitalCard(activeCardId);
};

// ==========================================
// 1. RESPONSIVE ID CARD PREVIEW MODAL
// ==========================================
...
function viewMemberDetails(membershipId) {
    const m = allMembers.find(x => x.membership_id === membershipId);
    if(!m) return;
    
    activeCardId = membershipId;
    ...
    const cardTarget = document.getElementById('digitalCardPrintArea');
    cardTarget.style.background = 'transparent';
    cardTarget.style.border = 'none';
    cardTarget.style.boxShadow = 'none';
    cardTarget.className = "w-full flex flex-col items-center mt-2 overflow-visible";
    ...
    cardTarget.innerHTML = `
        ...
        <div class="origin-top scale-[0.45] sm:scale-75 md:scale-90 lg:scale-100 transition-transform h-[180px] sm:h-[290px] md:h-[350px] lg:h-[380px] w-[600px] flex justify-center">
...
// ==========================================
// 2. PASSWORD ALERT POPUP
// ==========================================
function promptDownloadDigitalCard(membershipId) {
    const m = allMembers.find(x => x.membership_id === membershipId);
    if(!m) return;
    ...
    <div id="passwordAlertModal" class="fixed inset-0 z-[200] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
        <div class="bg-white rounded-3xl p-6 w-full max-w-md shadow-2xl border border-slate-200 animate-fade-in-up text-center">
            ...
            <div class="flex gap-3">
                ...
                <button onclick="executePDFDownload('${m.membership_id}'); document.getElementById('passwordAlertModal').remove()" class="flex-[1.5] bg-rose-600 hover:bg-rose-700 text-white font-bold py-3.5 rounded-xl shadow-md transition-colors text-xs uppercase tracking-wider"><i class="fa-solid fa-download mr-1"></i> Download PDF</button>
            </div>
        </div>
    </div>`;
...
// ==========================================
// 3. TWO-PAGE CARD-STYLE PDF GENERATOR (PRO)
// ==========================================
async function executePDFDownload(membershipId) {
    const m = allMembers.find(x => x.membership_id === membershipId);
    if(!m || !window.jspdf) {
        return spawnToastNotification("PDF Library not loaded yet.", "error");
    }

    toggleInteractionLoader(true, "Generating Card-Style PDF...");
    ...
    try {
        ...
        // Using a highly-reliable QR API that supports CORS
        ...
        exportContainer.innerHTML = `
            <div style="display: flex; flex-direction: column; gap: 20px;">
                ...
                <div id="pdf-back-card" style="width: 600px; height: 380px; position: relative; box-sizing: border-box; background: #ffffff; overflow: hidden; border-radius: 12px; border: 1px solid #cbd5e1;">
                    <img src="back.png" style="position: absolute; top: 0; left: 0; width: 600px; height: 380px; object-fit: cover; z-index: 1;">
        ...
        // This specific block catches the "Tainted Canvas / file:///" error
        try {
        ...
        spawnToastNotification("Secure Card PDF Downloaded Successfully!", "success");

    } catch (error) {
        console.error("PDF Export Error:", error);
        if (error.message === "SECURITY_ERROR") {
            spawnToastNotification("Browser blocked PDF. You MUST use a Local Web Server (not file:///).", "error");
        } else {
            spawnToastNotification("Failed to generate PDF. See console for details.", "error");
        }
    } finally {
